...
def apriori(
    ...
    [{a} -> {b}]
    """
    #itemsets_from_transactions has to be modified
    if spn:
        itemsets, num_trans = itemsets_from_transactions_SPN(
            transactions, min_support, max_length, verbosity, spn=spn,
        )
    else:
        itemsets, num_trans = itemsets_from_transactions(
            transactions, min_support, max_length, verbosity,
        )
    #this one doesn't need SPN specific modifications, i think
