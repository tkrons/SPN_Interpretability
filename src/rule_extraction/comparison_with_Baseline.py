...
lax_hyperparams = {'min_target_js': 0.1, 'min_global_conf': 'above_random',
...
min_sup = max(np.round(intra_df.sup.sort_values().head(3).mean() * 2, 2) , 0.02)
...
ap_topn = eval[eval.method.isin(['Apriori', 'SPN-Apriori'])].sort_values('F', ascending=False).groupby(
